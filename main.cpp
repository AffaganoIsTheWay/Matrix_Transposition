...
#ifdef _OPENMP
void transpose_parallel_static(int** matrix,int** transposed) {
...
void transpose_parallel_collapse_static(int** matrix,int** transposed) {
    // Parallelize both loops with static schedule
...
void transpose_parallel_collapse_dynamic(int** matrix,int** transposed) {
    // Parallelize both loops with dynamic schedule
...
void transpose_parallel_collapse_guided(int** matrix,int** transposed) {
    // Parallelize both loops with guided schedule
...
int main() {
    ...
    transpose_parallel_static(matrix, transposed_parallel);
    ...
    transpose_parallel_collapse_dynamic(matrix, transposed_parallel);

    end_parallel = omp_get_wtime();
    duration_parallel = (end_parallel - start_parallel);
    bandwidth_parallel = data_transferred / (duration_parallel * 1e9);

    cout << "Time taken by parallel Collapse Dynamic: " << duration_parallel << " seconds" << endl;
    cout << "Effective Parallel Bandwidth: " << bandwidth_parallel << " GB/s" << endl << endl;

    start_parallel = omp_get_wtime();

    transpose_parallel_collapse_guided(matrix, transposed_parallel);

    end_parallel = omp_get_wtime();
    duration_parallel = (end_parallel - start_parallel);
    bandwidth_parallel = data_transferred / (duration_parallel * 1e9);

    cout << "Time taken by parallel Collapse Guided: " << duration_parallel << " seconds" << endl;
    cout << "Effective Parallel Bandwidth: " << bandwidth_parallel << " GB/s" << endl << endl;
